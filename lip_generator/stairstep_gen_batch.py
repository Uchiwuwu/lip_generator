...
- Each trajectory has: wait_before + step + wait_mid + step2 + wait_after
...
def load_robot():
    ...
    half_sitting = np.array(
        [
            0,
            0,
            0.755,  # base position (updated for COM height ~0.57m) + 0.1 for stair
            0,
            0,
            0,
...
def main():
    ...
    for i, sample in enumerate(grid_samples):
        ...
        print(f"Step: {step['swing_foot']} swings (stance: {step['stance_foot']})")
        print(f"  Left foot: {lfPos0[:2]} -> {step['left_target'][:2]}")
        print(f"  Right foot: {rfPos0[:2]} -> {step['right_target'][:2]}")
        ...
        # Solve first step
        solver, success = solve_stepping_problem(gait, x0, step["left_target"], step["right_target"], step["target_yaw"], verbose=False)

        if not success or solver is None:
            continue
        ...
        try:
            step_data = extract_trajectory_data(robot, solver, gait, step["left_target"], step["right_target"])
        except Exception as e:
            print(f"✗ Failed to extract step data: {e}")
            del solver
            continue

        # Compute actual foot positions after step 1
        state_after_step1 = solver.xs[-1].copy()  # Copy to avoid reference to solver
        ...
        prev_stance = 0 if step["stance_foot"] == "left" else 1
        final_state = solver.xs[-1].copy()
        try:
            wait_data_after = generate_waiting_frames(
                robot, gait, final_state, wait_frames_after, prev_stance
            )
        except Exception as e:
        ...
        if step["swing_foot"] == "left":
            left_foot_first_count += 1
        else:
            right_foot_first_count += 1

        print(
            f"✓ Success! Wait before: {wait_frames_before}, Step1: {len(step_data['q'])}, "
            + f"Wait after: {wait_frames_after}"
        )
        ...
        if PLOT:
            if successful_samples % PLOT == 0:  # Plot every 5th successful sample
                ...
                traj_q = np.vstack([
                    wait_data_before["q"], step_data["q"],wait_data_after["q"]
                ])
                traj_v_b = np.vstack([
                    wait_data_before["v_b"], step_data["v_b"], wait_data_after["v_b"]
                ])
                traj_cmd_countdown = np.vstack([
                    wait_data_before["cmd_countdown"], step_data["cmd_countdown"], wait_data_after["cmd_countdown"]
                ])
                traj_com_rpy = np.vstack([
                    wait_data_before["qd"], step_data["qd"],  wait_data_after["qd"]
                ])
                traj_T_wbase = np.vstack([
                    wait_data_before["T_wbase"], step_data["T_wbase"],  wait_data_after["T_wbase"]
                ])
                ...
                traj_T_blf = np.vstack([
                    wait_data_before["T_blf"], step_data["T_blf"], wait_data_after["T_blf"]
                ])
                traj_T_brf = np.vstack([
                    wait_data_before["T_brf"], step_data["T_brf"], wait_data_after["T_brf"]
                ])
                traj_T_stsw = np.vstack([
                    wait_data_before["T_stsw"], step_data["T_stsw"], wait_data_after["T_stsw"]
                ])
                traj_cmd_footstep = np.vstack([
                    wait_data_before["cmd_footstep"], step_data["cmd_footstep"], wait_data_after["cmd_footstep"]
                ])
                traj_cmd_stance = np.vstack([
                    wait_data_before["cmd_stance"], step_data["cmd_stance"], wait_data_after["cmd_stance"]
                ])
